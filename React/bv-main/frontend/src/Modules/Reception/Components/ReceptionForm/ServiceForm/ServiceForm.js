...
import { FormGroup, Label, Row, Col, Input, Modal, ModalHeader, ModalBody, ModalFooter, Button, Table } from 'reactstrap';
...
export default class ServiceForm extends React.Component {
    ...
    handleChangeSelectServiceTest = (event, selectedService) => {
        ...
        if (this.props.items?.length > 0) {
            this.props.items[this.props.items?.length - 1]?.steps[0].order.items.map(e => {
                if (total[total.length - 1].id === e.ref_id && Date.now() - e.ctime < ONE_DAY) {
