...
class AccountingForm extends FormParent {
  constructor(props) {
    ...
    this.state = {
      bookList: [],
      paymentList: [
        { name: "Tiền mặt", method: "cash" },
        { name: "Thẻ ATM", method: "other" },
      ],
  ...
  setBookList = (bookList) => {
    this.setState({
      bookList: bookList.filter((el) => el.status === "active"),
    });
  ...
  handleSearch = (orderCode) => {
    if (this.props.retail) {
      ...
      ////// BÌNH THƯỜNG
      this.findOrderByCode(orderCode)
        .then((res) => {
          ...
          if (orderInfo.view_order.items.length == 1) {
            this._fillForm({
              ...
              nameService: orderInfo.view_order.items[0].ref_value.name,
              priceService: orderInfo.view_order.items[0].ref_value.price,
              cTime: new Date(orderInfo.view_order.ctime).toLocaleString(
                "en-GB"
              ),
  ...
  handleSearchByCustomerCode = () => {
    ...
    let items = [];
    let orderList = [];
    TransactionService.getListTransaction(customer_code)
      .then((res) => {
        let orderInfo = res.data;
        let orderInfoExam = res.data.filter(
          (e) => e.order.items[0]?.ref_value.type == "test"
        );
        let lastOrderInfoExam = orderInfoExam[orderInfoExam.length - 1];
        let orderInfoXray = res.data.filter(
          (e) => e.order.items[0]?.ref_value.type == "x-ray"
        );
        let lastOrderInfoXray = orderInfoXray[orderInfoXray.length - 1];
        let orderInfoEnt = res.data.filter(
          (e) => e.order.items[0]?.ref_value.type == "ent"
        );
        let lastOrderInfoEnt = orderInfoEnt[orderInfoEnt.length - 1];
        let orderInfoUltrasound = res.data.filter(
          (e) => e.order.items[0]?.ref_value.type == "ultrasound"
        );
        let lastOrderInfoUltrasound =
          orderInfoUltrasound[orderInfoUltrasound.length - 1];
        let orderInfoOther = res.data.filter(
          (e) => e.order.items[0]?.ref_value.type == "other"
        );
        ...
        if (lastOrderInfoUltrasound)
          lastOrderInfo.push(lastOrderInfoUltrasound);
  ...
  handleChangePaymentMethods = (event) => {
    ...
    this.setState({
      currentPaymentMethods: this.state.paymentList[index].method,
    });
  };
  ...
  handleSubmitCmCode = () => {
    ...
    const { bookId, currentPaymentMethods, note, items, orderList } =
      this.state;
  ...
  handleSubmit = () => {
    if (this.props.retail) {
      ...
      ReceptionService.postTransaction(data)
        .then((res) => {
          this.setNotiMessage("Thanh toán thành công!");
      ...
      if (billID && billID.length > 0) {
        ...
        ReceptionService.postTransaction(transaction)
          .then((res) => {
            this.setNotiMessage("Thanh toán thành công!");
            //FOR PHARMACY
            if (this.props.mode === "pharmacy") {
          ...
          <Col sx={12}>
            <Row className="customCard mt-10 pb-0 naV">
              <Col sm={6} className="row">
                <Label sm={2} className="title-card p-0" href="/">
                  <span className="material-icons">library_books</span> Sổ thu{" "}
                </Label>
                <Col sm={5}>
                  <Input
                    type="select"
              ...
              <Col sm={6} className="row">
                <Label sm={2} className="title-card order-code">
                  <span className="material-icons">playlist_add_check</span>Mã
                  hóa đơn
                </Label>
                <Col sm={5}>
                  <InputGroup>
                    ...
                    <InputGroupAddon addonType="append">
                      <Button
                        color="primary"
                        onClick={() => {
                          this.handleSearch(orderCode.value);
                        }}
                      >
                ...
                <Col sm={5}>
                  <InputGroup>
                    <Input
                      ...
                      onChange={(ev) => {
                        this._setValue(ev, "customerCode");
                      }}
                      type="text"
                      id="customerCode"
                      required
                    ...
                    <InputGroupAddon addonType="append">
                      <Button
                        color="primary"
                        onClick={() => {
                          this.handleSearchByCustomerCode(customer_code);
                        }}
                      >
                        <span className="material-icons">search</span>
                      </Button>
            ...
            <Form className="accounting-form">
              ...
              <FormGroup className="customCard info-form">
                <p className="title-card">
                  <span className="material-icons">description</span> Thông tin
                  đơn hàng
                </p>
                <FormGroup row>
                  <Col sm={12}>
                ...
                <FormGroup row>
                  <Label for="totalPrice" sm={4}>
                    Tổng số tiền giá gốc(bằng số)
                  </Label>
                  <Col sm={6}>
                    <NumberFormat
                      className="inputAc"
                      ...
                      suffix={" VND"}
                      displayType={"text"}
                      value={Math.ceil(this.state.origin_price)}
                    />
                  </Col>
                </FormGroup>
                <FormGroup row>
                  ...
                  <Col sm={6}>
                    <NumberFormat
                      className="inputAc"
                      onChange={(ev) => {
                        this._setValue(ev, "totalPrice");
                      }}
                      name="totalPrice"
                      thousandSeparator={true}
                      suffix={" VND"}
                      displayType={"text"}
                      value={Math.ceil(totalPrice.value)}
                    />
                  </Col>
                </FormGroup>
                <FormGroup row>
                  <Label for="wordPrice" sm={4}>
                ...
                <FormGroup row>
                  ...
                  <Col sm={6}>
                    <Input
                      ...
                      value={cTime.value}
                      onChange={(ev) => {
                        this._setValue(ev, "getTime");
                      }}
                    />
                  </Col>
                </FormGroup>
                <FormGroup row>
                  <Label for="getStatus" sm={4}>
              ...
              <FormGroup row className="customCard money-form">
                <Label sm={3} className="title-card">
                  <span className="material-icons">monetization_on</span> Thanh
                  toán hóa đơn
                </Label>
                <Col sm={2}>
                  <Input
                  ...
                  >
                    {this.state.paymentList.map((item, index) => {
                      return (
                        <option key={item.method} value={index}>
                          {item.name}
                        </option>
                      );
                    })}
             ...
             <AccountingPrintingPreview
                ...
                customerAddress={customerAddress}
                customerPhone={this.state.customerPhone1}
              ></AccountingPrintingPreview>
          </Col>
        )}
        {mode === "pharmacy" && (
          <Col sx={12}>
            <Row className="customCard mt-3 pb-0">
              ...
              <Col sm="6" className="row">
                <Label sm={4} className="title-card">
                  <span className="material-icons">playlist_add_check</span>Mã
                  hóa đơn
                </Label>
                <Col sm={8}>
