...
class PrintResultExam extends Form {
    ...
    componentDidUpdate = (preProps) => {
        let { cusData, resultsExam, tab, jobStep,idPrint } = this.props;
        if (preProps.resultsExam !== resultsExam) {
            if(idPrint == "print_Result"){
                if (cusData?.code) {
                    resultsExam?.map((el, index) => {
                        SharedService.createOtccode(cusData?.code, "cusBarCodeExam" + index)
                    })
                }
                SharedService.getUserById(jobStep.modified_by).then(res => {
                    this.setState({
                        modifiedUser: res.data
                    })
                })
                SharedService.getUserById(jobStep.created_by).then(res => {
                    this.setState({
                        doctorInfo: res.data
                    })
                })
    ...
    render() {
        ...
        const { modifiedUser, doctorInfo } = this.state;
        const ctime = new Date(this.props.jobStep.ctime).toLocaleString('en-GB');
                            ...
                            <div className="print_Result">
                                ...
                                <Row className="cusInfo">
                                    ...
                                    <Col className='asign-right'>
                                        ...
                                        Người nhận mẫu : {modifiedUser?.full_name}<br></br>
                                ...
                                {tab == 1 ?
                                    ...
                                    <div className="d-flex justify-content-between footer-print px-4">
                                        <div className="text-center left-footer mr-auto ml-5 ">
                                            <Signature  tab={tab}></Signature>
                                        </div>
                                        <div className="text-center right-footer">
                                            <div>Bắc Ninh, {ctime}</div>
                                            <strong>PHÒNG XÉT NGHIỆM</strong>
                                            <br></br><br></br><br></br>
                                            <b className="mt-5">{modifiedUser.full_name}</b>
                                        </div>
                                    </div>
                                }
